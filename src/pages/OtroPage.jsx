...
// Importación de tus datos agrupados
import { FIGMA_RESOURCES } from "../data/resourcesData";

// --- COMPONENTE TARJETA ---
const ResourceCard = ({ res, copiedId, onCopy }) => {
  return (
    <motion.div
      layout
    ...
    >
      <div className="bg-raisin/40 rounded-[22px] p-6 h-full flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-taupe text-xs leading-relaxed">{res.desc}</p>
              </div>
            </div>
            
            {res.link && (
              <a href={res.link} target="_blank" rel="noreferrer" className="p-2 hover:bg-white/5 rounded-full transition-colors">
                <ExternalLink size={16} className="text-taupe/40 group-hover:text-white" />
              </a>
            )}
          </div>
          ...
          {res.code && (
            <div className="mt-4 space-y-4">
              <div className={`h-28 rounded-xl ${res.preview} flex items-center justify-center text-[10px] uppercase tracking-[0.3em] opacity-60 border border-white/5`}>
                Preview
              </div>
        ...
        {res.code && (
          <button
            onClick={() => onCopy(res.code, res.id)}
          ...
          >
            ...
            {copiedId === res.id ? "COPIADO CON ÉXITO" : "COPIAR SNIPPET"}
          </button>
        )}
      </div>
    </motion.div>
...
const ProgrammingPage = () => {
  ...
  // Otros recursos que no son plugins de Figma
  ...
  return (
    <div className="w-full h-screen bg-raisin text-white flex overflow-hidden">
          ...
          >
            <div className="relative z-10 p-8 flex flex-col h-full bg-gradient-to-b from-black/50 to-transparent">
              <div className="flex items-center gap-3 mb-12">
                ...
                <span className="font-gugi text-xl tracking-tighter text-golden">FIGMA.HUB</span>
              </div>

              <nav className="space-y-4 flex-1">
                {[
                  { icon: <LayoutDashboard size={20} />, label: "Dashboard", cat: "all" },
                  { icon: <Globe size={20} />, label: "Links", cat: "links" },
                  { icon: <Cpu size={20} />, label: "Plugins", cat: "plugins" },
                  { icon: <Code2 size={20} />, label: "Snippets", cat: "snippets" },
                ].map((item, i) => (
                  <motion.div
                    key={i}
                    onClick={() => setActiveCategory(item.cat)}
                    whileHover={{ x: 10, color: "#D4AF37" }}
                    className={`flex items-center gap-4 p-3 rounded-xl cursor-pointer transition-colors border border-transparent ${activeCategory === item.cat ? 'bg-golden/10 border-golden/20 text-golden' : 'text-taupe/80 hover:bg-golden/5'}`}
                  >
                    {item.icon}
                    <span className="font-anta text-sm uppercase tracking-widest">{item.label}</span>
                  </motion.div>
                ))}
              </nav>
      ...
      {/* CONTENIDO PRINCIPAL */}
      <main className="flex-1 overflow-y-auto custom-scrollbar bg-raisin">
        <header className="p-8 flex flex-col md:flex-row gap-6 items-center justify-between border-b border-white/5 bg-raisin/50 backdrop-blur-xl sticky top-0 z-[100]">
          <div className="flex items-center gap-4">
            ...
            <h2 className="font-gugi text-xl uppercase tracking-widest text-white/50">Recursos</h2>
          </div>
        ...
        <div className="p-8 lg:p-12 space-y-16">
          ...
          {/* SECCIÓN 1: FIGMA PLUGINS (Agrupados por categoría) */}
          {(activeCategory === "all" || activeCategory === "plugins") && (
            <div className="space-y-12">
              <div className="flex items-center gap-4">
                <h3 className="font-gugi text-lg text-golden/40 uppercase tracking-[0.2em]">Figma Plugins</h3>
                <div className="h-[1px] flex-1 bg-golden/10"></div>
              </div>

              {Object.entries(FIGMA_RESOURCES).map(([groupName, items]) => {
                ...
                return (
                  <div key={groupName} className="space-y-6">
                    <h4 className="font-anta text-xs text-taupe/40 uppercase tracking-[0.4em] pl-4 border-l border-golden/30">
                      {groupName}
                    </h4>
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                      <AnimatePresence mode="popLayout">
                        {filteredItems.map((plugin) => (
                          <ResourceCard key={plugin.id} res={plugin} />
                        ))}
                      </AnimatePresence>
                    </div>
                  </div>
          ...
          {/* SECCIÓN 2: OTROS RECURSOS (Links y Snippets) */}
          {(activeCategory === "all" || activeCategory === "links" || activeCategory === "snippets") && (
            <div className="space-y-12 pt-8">
              <div className="flex items-center gap-4">
                <h3 className="font-gugi text-lg text-white/20 uppercase tracking-[0.2em]">
                  {activeCategory === "all" ? "Desarrollo & Web" : activeCategory}
                </h3>
